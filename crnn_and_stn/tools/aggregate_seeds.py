...
def summarize(values: list[float], label: str, n_val: int) -> tuple[float, float]:
...
def main() -> None:
    ...
    mean, std = summarize(values, args.label, args.val_size)

    if args.baseline_acc:
        base_mean, base_std = summarize(args.baseline_acc, args.baseline_label, args.val_size)
